...
@basketball_bp.route('/NBA')
def index():
    redis_client = current_app.extensions["redis"]
    games_data = redis_client.get("nba_data")
    NBA_games =[]
    ...
    if games_data:
        ...
        NBA_games = calculate_best_odds_and_books(NBA_games)

    else:
        NBA_games =[]
    return render_template('basketball/basketball.html', games=NBA_games)

@basketball_bp.route('/game/<string:game_id>/')
def game(game_id):
    redis_client = current_app.extensions["redis"]
    games_data = redis_client.get("nba_data")
